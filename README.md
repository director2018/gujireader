...
把扫描文档按**现代左开本**的方式阅读：第 1 页在最左侧，翻页时页面向右推进，
左右按钮可直接前后翻页。支持更换阅读底色（五种护眼配色）、手抓拖动、最近打开记录。
也保留**古籍右开本**（自右向左翻）可随时在顶栏切换。
...
| 方向 | 含义 |
| --- | --- |
| **左开本**（默认） | 现代装帧。第 1 页在最左端，自左向右推进（向右翻） |
| **右开本** | 古籍装帧。第 1 页在最右端，翻页时页面自右向左推进 |
...
- **左开本**：滑块起步停在**最左端**（此刻看到的正是第 1 页），
  往**右**拖才是往后读，一路拖到最右端就是末页。
- **右开本**：滑块起步在最右端，往左拖往后读。
